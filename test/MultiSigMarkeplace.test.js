...
const { getMockAllAdmins, getMockNbAdmins } = require("../test/content/mockdata/fetcher");
...
describe("Multisig contract deployment test", function () {
    describe("constructor", () => {
        it("Initialize multi-sig contract", async () => {
            ...
            const ownerAddresses = await deployedMultisigContract.getOwnersAddresses();
    ...
    describe("Test Multisig Submit/Confirm/Execute functions", () => {
        ...
        before(async function () {
            ...
            deployedDemoContract = await demoContract
                .connect(user1)
                .deploy([user1.address, user2.address], deployedMultiSigContract.address);
        });
        ...
        it("Deployer(user1) from demo contract CANNOT blacklist user2", async () => {
            await expect(deployedDemoContract.connect(user1).freezeUser(user2.address)).to
                .be.reverted;
        });
    ...
    describe("Test AddOwner and RemoveOwner functions", () => {
        ...
        it("Deployer of the multisig contract CANNOT add/remove a owner", async () => {
            await expect(deployedMultiSigContract.connect(owner2).addOwner(user1.address))
                .to.be.reverted;
            await expect(
        ...
        it("Random account CANNOT add/remove a owner", async () => {
            await expect(deployedMultiSigContract.connect(user2).addOwner(user1.address))
                .to.be.reverted;
            await expect(
        ...
        it("Adding a new owner can only be achieved by submitting a new proposal", async () => {
            ...
            assert(allOwnersAfterProposalExecuted.length, ownersInMultiSigContract + 1);
